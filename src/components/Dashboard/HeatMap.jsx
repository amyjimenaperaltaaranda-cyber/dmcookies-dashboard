      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-2">
        <div>
          <h3 className="text-base font-bold text-white tracking-tight">
            Mapa de Calor — Rastreo
          </h3>
          <p className="text-sm text-slate-400 mt-1">
            Intensidad de rastreo por trimestre y mes
          </p>
          ...
          <details className="mt-2 group">
            <summary className="text-xs font-semibold text-[#e0973f] cursor-pointer hover:underline outline-none flex items-center gap-1">
              ...
              <span className="group-open:rotate-180 transition-transform duration-300 text-[10px]">
                ▼
              </span>
            </summary>
            ...
            <p className="text-[11px] leading-relaxed text-slate-300 mt-2 bg-slate-900/60 p-3 rounded-lg border border-slate-700/50 max-w-sm shadow-inner z-50 relative">
              Visualiza la intensidad del seguimiento web a los usuarios. Los colores
              más intensos (naranjas brillantes) revelan épocas del año con tasas de
              rastreo más agresivas mediante cookies y scripts de terceros.
            </p>
          </details>
        </div>
          ...
          <div className="flex gap-1 mx-1">
            {[10, 30, 50, 70, 100].map((o) => (
              <div
                ...
                style={{ backgroundColor: `rgba(249,115,22,${o / 100})` }}
              />
            ))}
          </div>
            ...
            <div className="flex mb-2 shrink-0">
              ...
              <div className="flex-1 grid grid-cols-12 gap-1">
                {meses.map((m) => (
                  <div
                    key={m}
                    className="notranslate text-center text-[10px] sm:text-[11px] text-slate-500 font-medium tracking-tighter overflow-hidden text-ellipsis"
                    translate="no"
                  >
                    {m}
                  </div>
            ...
            <div className="flex-1 flex flex-col gap-2">
              {trimestres.map((q) => (
                <div key={q} className="flex-1 flex items-center">
                  <div
                    className="notranslate w-10 sm:w-12 text-[10px] sm:text-[11px] text-slate-400 font-bold shrink-0 leading-tight"
                    translate="no"
                  >
                    {q}
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-12 gap-1 h-full">
                    {meses.map((m) => {
                      ...
                      return (
                        <div
                          key={`${q}-${m}`}
                          className={`rounded border transition-all duration-200 cursor-pointer group relative ${
                            val !== null
                              ? getColorClass(val, maxVal)
